...
const Playlist = () => {
  ...
  if (songs.length === 0) {
    return (
      <div className="h-[60vh] flex flex-col items-center justify-center text-neutral-500">
        ...
        <h2 className="text-3xl font-extrabold text-white mb-3 tracking-tight">Your Library is Empty</h2>
  ...
  return (
    <div className="w-full pt-2">
      <div className="fixed right-2 top-1/2 -translate-y-1/2 flex flex-col items-center gap-[2px] z-50 bg-black/40 backdrop-blur-md py-3 px-1.5 rounded-full border border-white/10 shadow-xl hidden md:flex">
          ...
          return (
            <button
              ...
              className={`text-[9px] font-bold w-[18px] h-[18px] flex items-center justify-center rounded-full transition-all ${
                hasLetter 
                  ? 'text-neutral-400 hover:text-white hover:bg-cyan-500/50 cursor-pointer hover:scale-125' 
                  : 'text-neutral-700/50 cursor-default'
              }`}
            >
              {letter}
            </button>
      ...
      <div className="grid grid-cols-[auto_1fr_auto_auto] gap-4 px-6 py-4 border-b border-white/5 text-[11px] font-bold text-neutral-500 uppercase tracking-widest sticky top-0 bg-[#0a0a0a]/80 backdrop-blur-xl z-40">
          ...
          return (
            <div 
              ...
              className={`group grid grid-cols-[auto_1fr_auto_auto] gap-4 px-6 py-3 cursor-pointer transition-all duration-300 rounded-xl items-center
                ${isActive ? 'bg-white/10 shadow-[0_4px_20px_rgba(0,0,0,0.5)]' : 'hover:bg-white/5'}`}
            >
              ...
              <div className="flex items-center gap-4 overflow-hidden pr-4">
                <div className={`w-11 h-11 rounded-md shadow-md overflow-hidden bg-neutral-800 flex-shrink-0 transition-transform duration-300 ${isActive ? 'scale-105 shadow-cyan-500/20' : 'group-hover:scale-105'}`}>
                  {song.albumArt ? (
                ...
                <div className="overflow-hidden">
                  <p className={`font-bold truncate text-[15px] tracking-tight transition-colors ${isActive ? 'text-cyan-400' : 'text-neutral-100 group-hover:text-white'}`}>
                    {song.title}
                  </p>
                  <p className="text-[13px] text-neutral-400 truncate mt-0.5 font-medium">{song.artist}</p>
                </div>
              </div>
              
              <div className="hidden md:flex items-center w-48 lg:w-64 text-[13px] text-neutral-400 truncate pr-4">
                <span className="truncate hover:text-white transition-colors cursor-pointer">{song.album}</span>
              </div>
              
              <div className="w-16 flex items-center justify-end text-[13px] text-neutral-400 font-mono tracking-tighter">
